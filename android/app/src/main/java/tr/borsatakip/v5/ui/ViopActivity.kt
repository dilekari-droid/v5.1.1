...
class ViopActivity : BaseActivity() {
    ...
    private fun bindUnderlyingIndex(stock: Stock?, valueId: Int, changeId: Int, chartId: Int) {
        ...
        val previous = stock?.previousClose ?: stock?.candles?.let { candles ->
            if (candles.size >= 2) candles[candles.lastIndex - 1].close else null
        }
    ...
    private fun refreshProviderState() {
        ...
        if (live) {
            ...
            providerStatus.text = "${previewMetadata?.source ?: "Production backend"} • ${MarketDataQuality.uiStatus(previewMetadata)} • son başarılı ${timeOf(previewMetadata?.lastSuccessfulUpdateAt ?: 0L)}"
        } else {
    ...
    private fun bindDashboardFilters() {
        ...
        listOf(
            R.id.filterIndex to UiCategory.INDEX,
            R.id.filterEquity to UiCategory.EQUITY,
            R.id.filterFx to UiCategory.FX,
            R.id.filterCommodity to UiCategory.COMMODITY,
            R.id.filterRate to UiCategory.RATE
        ).forEach { (id, category) ->
        ...
        if (showingUnderlying) {
            ...
            val filtered = source.asSequence()
                .filter { candidate ->
                    ...
                    val directionOk = when (uiDirection) {
                        UiDirection.ALL -> true
                        UiDirection.LONG -> bias == ViopSignalPolicy.AnalysisBias.LONG
                        UiDirection.SHORT -> bias == ViopSignalPolicy.AnalysisBias.SHORT
                        UiDirection.WATCH -> bias == ViopSignalPolicy.AnalysisBias.NEUTRAL
                    }
                    searchOk && categoryMatches(contract.underlying) && directionOk
                }
                .sortedWith(when (uiSort) {
                    UiSort.RANKING -> compareByDescending<ViopUnderlyingScanner.Candidate> { it.underlying.rankingScore }.thenByDescending { it.underlying.finalSignalScore }
                    UiSort.SIGNAL -> compareByDescending<ViopUnderlyingScanner.Candidate> { it.underlying.finalSignalScore }.thenByDescending { it.underlying.rankingScore }
                    UiSort.CONFIDENCE -> compareByDescending<ViopUnderlyingScanner.Candidate> { it.underlying.dataConfidenceScore }.thenByDescending { it.underlying.rankingScore }
            ...
            val filtered = source.asSequence()
                .filter { opportunity ->
                    ...
                    val directionOk = when (uiDirection) {
                        UiDirection.ALL -> true
                        UiDirection.LONG -> published == ViopSignalPolicy.PublishedDirection.LONG
                        UiDirection.SHORT -> published == ViopSignalPolicy.PublishedDirection.SHORT
                        UiDirection.WATCH -> published == ViopSignalPolicy.PublishedDirection.WATCH
                    }
                    searchOk && categoryMatches(c.underlying) && directionOk
                }
                .sortedWith(when (uiSort) {
                    UiSort.RANKING -> compareByDescending<ViopOpportunity> { it.rankingScore }.thenByDescending { it.finalScore }
                    UiSort.SIGNAL -> compareByDescending<ViopOpportunity> { it.finalScore }.thenByDescending { it.rankingScore }
                    UiSort.CONFIDENCE -> compareByDescending<ViopOpportunity> { it.dataConfidenceScore }.thenByDescending { it.rankingScore }
    ...
    private fun categoryMatches(underlyingRaw: String): Boolean {
        ...
        return when (uiCategory) {
            UiCategory.ALL -> true
            UiCategory.INDEX -> underlying.startsWith("XU") || underlying.contains("BIST")
            UiCategory.EQUITY -> !(underlying.startsWith("XU") || underlying.contains("BIST") || underlying.contains("USD") || underlying.contains("EUR") || underlying.contains("TRY") || listOf("GOLD", "SILVER", "BRENT", "XAU", "XAG").any { underlying.contains(it) } || underlying.contains("RATE") || underlying.contains("FAIZ") || underlying.contains("TLREF"))
            UiCategory.FX -> underlying.contains("USD") || underlying.contains("EUR") || underlying.contains("TRY")
            UiCategory.COMMODITY -> listOf("GOLD", "SILVER", "BRENT", "XAU", "XAG").any { underlying.contains(it) }
            UiCategory.RATE -> underlying.contains("RATE") || underlying.contains("FAIZ") || underlying.contains("TLREF")
        }
    }
    ...
    private fun updateDashboardFilterColors() {
        val allSelected = uiCategory == UiCategory.ALL && uiDirection == UiDirection.ALL
        ...
        tint(R.id.filterShort, uiDirection == UiDirection.SHORT, getColor(R.color.red))
        tint(R.id.filterWatch, uiDirection == UiDirection.WATCH, Color.parseColor("#6E8FB2"))
        tint(R.id.filterIndex, uiCategory == UiCategory.INDEX, getColor(R.color.blue))
        tint(R.id.filterEquity, uiCategory == UiCategory.EQUITY, getColor(R.color.blue))
        tint(R.id.filterFx, uiCategory == UiCategory.FX, getColor(R.color.blue))
        tint(R.id.filterCommodity, uiCategory == UiCategory.COMMODITY, getColor(R.color.blue))
        tint(R.id.filterRate, uiCategory == UiCategory.RATE, getColor(R.color.blue))
    }
    ...
    private enum class UiSort { RANKING, SIGNAL, CONFIDENCE }
