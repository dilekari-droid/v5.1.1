...
class MobileMarketDataProvider(context: Context) : MarketDataProvider, ScanDiagnosticsSource, ScanBatchTimeframeConfigurable {
    ...
    private val client = OkHttpClient.Builder()
        ...
        .build()
    private data class EffectiveBatchPolicy(
        val maxSymbols: Int,
    ...
    private suspend fun loadBatchPolicy(): EffectiveBatchPolicy {
        val remote = MarketCapabilityClient(appContext).load().getOrNull()?.batchPolicy
        val maxSymbols = (remote?.maxSymbols ?: DEFAULT_BATCH_SIZE).coerceIn(1, 100)
        val readTimeout = (remote?.readTimeoutMs ?: DEFAULT_BATCH_READ_TIMEOUT_MS).coerceIn(30_000L, 600_000L)
        val callTimeout = (remote?.callTimeoutMs ?: DEFAULT_BATCH_CALL_TIMEOUT_MS).coerceIn(readTimeout, 600_000L)
        val outerTimeout = (remote?.outerTimeoutMs ?: DEFAULT_BATCH_OUTER_TIMEOUT_MS).coerceIn(callTimeout, 650_000L)
        return EffectiveBatchPolicy(maxSymbols, readTimeout, callTimeout, outerTimeout)
    }

    private fun batchClient(policy: EffectiveBatchPolicy): OkHttpClient = client.newBuilder()
