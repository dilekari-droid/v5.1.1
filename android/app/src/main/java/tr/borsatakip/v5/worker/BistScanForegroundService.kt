...
class BistScanForegroundService : Service() {
    ...
    private fun notification(session: ManualScanSession, ongoing: Boolean): Notification {
        ...
        val countText = if (total > 0) "${session.scannedCount.coerceAtMost(total)} / $total • %${session.progress}" else session.phase
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setProgress(total.coerceAtLeast(0), session.scannedCount.coerceIn(0, total.coerceAtLeast(0)), total <= 0 && session.isActive)
