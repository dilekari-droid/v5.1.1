...
object MarketDataQuality {
    ...
    const val MAX_LIVE_AGE_MS = 90_000L
    const val MAX_DELAYED_AGE_MS = 4L * 24L * 60L * 60L * 1000L
    ...
    fun uiStatus(metadata: MarketDataMetadata?): String = when (metadata?.state) {
        MarketDataState.LIVE -> "CANLI"
        MarketDataState.DELAYED -> "GECİKMELİ"
        MarketDataState.FALLBACK -> "YEDEK KAYNAK"
        MarketDataState.STALE -> "ESKİ VERİ"
        MarketDataState.OFFLINE -> "ÇEVRİMDIŞI"
        else -> "DOĞRULANMADI"
    }
}
