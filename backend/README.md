...
Production Docker deployments set `APP_ENV=production`; startup fails unless `APP_API_KEY`, a strong `SESSION_TOKEN_SECRET` (minimum 32 characters by default), and TradeWize credentials are present. `SESSION_TOKEN_KEY_ID`, `SESSION_TOKEN_PREVIOUS_KEYS_JSON` and `SESSION_TOKEN_NOT_BEFORE` provide deterministic rotation overlap/boundaries; subject-epoch revocation can be persisted with `SESSION_REVOKE_STORE_PATH`. The conservative production default `UPSTREAM_QUOTA_SCOPE=all` routes TradeWize market-data GETs through the same minute/hour pacing and bounded retry gate; `bars_only` is available only for deployments that have independently verified a narrower provider quota contract. Upstream HTTP 429 and HTTP 503 responses carrying `Retry-After` extend the same process-wide market-data cooldown, and stale-quote `recent-ticks` recovery uses that shared gate. A rolling bar cache reuses verified closed candles and supports incremental windows while bounding idle age, key count and total candle count. One `CacheFreshnessPolicy` controls open/unknown-session, observed-closed-session and daily-history TTLs (`BAR_CACHE_TTL_SECONDS`, `BAR_CACHE_CLOSED_SESSION_TTL_SECONDS`, `BAR_CACHE_DAILY_TTL_SECONDS`) so history routes cannot drift into endpoint-specific stale rules. `range=max` returns the untruncated normalized upstream result while only the cached tail is bounded. Android→backend ingress has a separate layered token-bucket guard with TTL/LRU-bounded identity state. VİOP contracts remain fail-closed unless `TRADEWIZE_VIOP_CONTRACT_METADATA_PATH` points to a verified metadata source. Metadata pagination follows `hasMore/nextCursor` with duplicate/cursor-loop/snapshot-consistency guards and bounded `VIOP_METADATA_PAGE_SIZE` / `VIOP_METADATA_MAX_PAGES`. Installation-bound short-lived `BorsaSession` tokens are mandatory in production configuration and remain optional only in non-production development/test environments.


Provider request-weight values are never invented in source. Until TradeWize publishes/validates the applicable formula, `providerWeightFormulaVerified=false` remains fail-closed. After independent verification, operators may provide the official source/version plus `TRADEWIZE_REQUEST_WEIGHT_FORMULA_JSON` and explicitly set `TRADEWIZE_REQUEST_WEIGHT_FORMULA_VERIFIED=true`; startup rejects incomplete/invalid verified formula configuration.
