...
INSTALL = os.getenv("BORSA_E2E_INSTALL_ID", "ci-e2e-v5416").strip()
...
def _artifact_payload(checks: list[Check], *, started_at: str, elapsed_ms: int, error: str | None = None, deployment_revision: str = "", failure_stage: str = "") -> dict[str, Any]:
    return {
        "schemaVersion": 2,
        "appVersion": "5.4.16",
...
def main() -> int:
    ...
    try:
        ...
        with httpx.Client(base_url=BASE, headers=headers, timeout=TIMEOUT, follow_redirects=False) as client:
            ...
            if preflight_realtime:
                ...
                for row in items:
                    ...
                    if not realtime or ts <= 0 or now_ms - ts > max_age_ms:
                        stale.append(str(row.get("symbol")))
                if stale:
                    raise AssertionError("preflight REALTIME but snapshot-batch contains non-live/stale quotes: " + ",".join(stale))
                checks.append(Check("preflight-batch-freshness-parity", True, f"rows={len(items)}"))
            else:
                checks.append(Check("preflight-batch-freshness-parity", True, "preflight not realtime; no realtime claim asserted"))
            ...
            if viop_ready:
                ...
                if str(first.get("contractType")).upper() != "FUTURE" or missing:
                    raise AssertionError(f"invalid VIOP contract metadata: missing={missing} type={first.get('contractType')}")
    ...
    _write_artifact(_artifact_payload(checks, started_at=started_at, elapsed_ms=elapsed_ms, error=error, deployment_revision=deployment_revision, failure_stage=failure_stage if error else ""))
