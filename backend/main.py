...
async def ingress_rate_limit_middleware(request: Request, call_next):
    ...
    if authorization.startswith("BorsaSession "):
        ...
        if not _verify_session_token(token, expected_installation_id=install_id):
...
def _issue_session_token(installation_id: str) -> tuple[str, int]:
    ...
    payload = {
        ...
        "sub": subject,
        "subjectEpoch": int(_session_subject_epochs.get(subject, 0)),
        "iat": now,
        "exp": exp,
...
def _verify_session_token(token: str, expected_installation_id: str | None = None) -> bool:
    ...
    if not subject or not hmac.compare_digest(subject, expected_subject):
        return False
    if subject_epoch != int(_session_subject_epochs.get(subject, 0)):
        return False
...
def require_app_auth(authorization: str | None) -> None:
    ...
    if authorization == f"Bearer {APP_API_KEY}":
        return
    if authorization and authorization.startswith("BorsaSession ") and _verify_session_token(authorization.removeprefix("BorsaSession ").strip()):
        return
...
async def create_backend_session(
    ...
    if not installation or len(installation) > 128:
        raise HTTPException(status_code=400, detail="INSTALLATION_ID_REQUIRED")
    token, exp = _issue_session_token(installation)
    return {
...
async def provider_capabilities(force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        result["features"]["viopContractsReady"] = metadata_ready
        market_ready = live_ready and metadata_ready
        result["markets"]["VIOP"] = {
            "supported": True, "discovery": metadata_ready, "marketData": True, "historicalData": True, "symbolCount": len(viop), "usableCount": len(viop),
            "liveCount": sum(1 for item in viop if item.get("realtime")), "realtime": live_ready, "realtimeReady": live_ready, "analysisReady": bool(viop),
            "contractsReady": metadata_ready, "ready": market_ready, "provider": "TradeWize",
            "reasonCode": None if market_ready else ("VIOP_CONTRACT_METADATA_UNAVAILABLE" if not metadata_ready else "STALE_DATA"),
            "message": "VİOP canlı fiyat ve kontrat metadata doğrulandı." if market_ready else (metadata_reason or f"VİOP discovery hazır; canlı quote doğrulanmadı ({live_reason})."),
        }
    except Exception as exc:
        reason = str(exc.detail) if isinstance(exc, HTTPException) else f"{type(exc).__name__}: {exc}"
    ...
    for meta in page_meta:
        ...
        items.append({
            **meta,
            ...
            "openInterest": None, "volume": None, "liquidity": None, "rollover": None,
...
def scanner_symbols(
    ...
    viop_candidates = [item["symbol"] for item in _viop_cache.get("items", []) if isinstance(item, dict) and item.get("symbol")]
...
async def _scan_symbol(symbol: str, interval: str, market: str) -> tuple[dict[str, Any] | None, str | None]:
    try:
        ...
        item = _scan_from_bars(symbol, bars, interval, {"market": market, "assetType": "STOCK" if market == "BIST" else "FUTURE"})
    ...
    try:
        ...
        for row in rows:
            ...
            items.append({
                ...
                "signalTime": int(row["signal_time"]), "receivedAt": int(row["received_at"]),
