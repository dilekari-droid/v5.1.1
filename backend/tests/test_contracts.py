...
async def test_capabilities_viop_symbol_count_no_nameerror(monkeypatch):
    ...
    async def fake_viop(force=False):
        ...
        x.update({"market": "VIOP", "assetType": "FUTURE"})
...
def test_android_batch_timeout_matches_backend_queued_pacing_budget():
    ...
    assert "DEFAULT_BATCH_READ_TIMEOUT_MS = 175_000L" in source
    assert "DEFAULT_BATCH_CALL_TIMEOUT_MS = 180_000L" in source
    assert "DEFAULT_BATCH_OUTER_TIMEOUT_MS = 185_000L" in source
...
def test_android_session_refresh_is_single_flight_and_renews_early():
    ...
    assert "private val refreshMutex = Mutex()" in client
    assert "refreshMutex.withLock" in client
...
def test_research_backend_and_fallback_result_sources_are_explicit_in_android_model():
    repo_root = Path(__file__).resolve().parents[2]
    models = (repo_root / "android" / "app" / "src" / "main" / "java" / "tr" / "borsatakip" / "v5" / "research" / "ResearchModels.kt").read_text()
    api = (repo_root / "android" / "app" / "src" / "main" / "java" / "tr" / "borsatakip" / "v5" / "data" / "ResearchApiClient.kt").read_text()
    repository = (repo_root / "android" / "app" / "src" / "main" / "java" / "tr" / "borsatakip" / "v5" / "data" / "ResearchFoundationRepository.kt").read_text()
    assert "enum class ResearchResultSource" in models
    assert "BACKEND" in models and "FALLBACK" in models
    assert "resultSource = ResearchResultSource.BACKEND" in api
    assert "resultSource = ResearchResultSource.FALLBACK" in repository
...
def test_public_health_but_protected_symbols_remain_auth_required(monkeypatch):
    """Railway liveness is public, while protected market-data routes remain fail-closed."""
    from fastapi.testclient import TestClient
    import main

    monkeypatch.setattr(main, "APP_ENV", "production")
    monkeypatch.setattr(main, "APP_API_KEY", "x" * 32)
    client = TestClient(main.app)
...
def test_viop_filter_sort_state_survives_activity_recreation_source_contract():
    repo_root = Path(__file__).resolve().parents[2]
    ...
    assert "restoreUiState(savedInstanceState)" in source
    for key in ("STATE_QUERY", "STATE_CATEGORY", "STATE_DIRECTION", "STATE_SORT", "STATE_SHOWING_UNDERLYING", "STATE_ADVANCED_FILTER"):
